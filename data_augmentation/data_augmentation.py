...
def main():
    input_path  = Path(os.environ.get("CARPETA_ORIGEN",  ""))
    output_path = Path(os.environ.get("CARPETA_DESTINO", ""))
    iniciar_en  = int(os.environ.get("INICIAR_EN", "1"))
    cantidad    = int(os.environ.get("CANTIDAD",   "10"))

    if not input_path or not input_path.is_dir():
        print(f"Error: CARPETA_ORIGEN ('{input_path}') no es una carpeta válida.")
        return

    if not output_path or str(output_path) == ".":
        print("Error: CARPETA_DESTINO no está definida en el .env.")
        return
    ...
    print(f"Iniciando desde:      {iniciar_en}")
    print(f"Aumentos por imagen:  {cantidad}\n")

    for i, image_path in enumerate(images):
        index = iniciar_en + i
        print(f"Procesando: {image_path.name} -> {base_name}_{index}")
        try:
            process_image(image_path, output_path, base_name, index, cantidad)
